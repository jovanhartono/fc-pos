...
**Category**:
Grouping for Services and Products. Used in reports (Gross sales is reported as Store × Category).
...
**Complaint**:
A Customer's grievance about a finished `OrderService` — the Item was cleaned poorly, they are dissatisfied. Usually raised at the counter: the cashier shows the customer the ready pair before handing it over, and the customer turns it down there (the line is still `ready_for_pickup`); sometimes raised after they took it home (`picked_up`). A first-class, **write-once** record: `order_service_id` FK → the original (complained) line, free-text `reason`, `opened_by`, `created_at` — that is the whole row. **One Complaint per original line, lifetime** (`order_service_id` is UNIQUE): a Complaint is the whole grievance *episode*, not one round — "still bad after a Rework" is handled *inside* it (another Rework, or escalate to refund). A Complaint row is an **already-accepted** grievance — there is no "rejected" outcome and no open/closed status. Owns **0..N** Rework lines. Handling is an **escalation ladder** — re-clean first (a Rework); if still unsatisfactory, an admin refunds the original line (the money gate). The Complaint's **outcome is derived, never stored**: *reworked* if a Rework line that is not `cancelled` points at it, *refunded* if the original line is `refunded`, *cancelled* if it is `cancelled` (turned down at the counter on an unpaid Order and cancelled instead of reworked), else *pending*; *refunded* and *cancelled* win over *reworked*. Opening a Complaint and adding a Rework are open to **any staff** (the cashier takes the return); refunding is **admin-only** (the existing money gate, on the refund flow). Opening requires a finished line — `picked_up`, or `ready_for_pickup` once every live treatment on its Item is ready too (the customer is shown the whole pair) — and rejects a subject that is itself a Rework line; adding a Rework requires the original line still be `ready_for_pickup` or `picked_up` (refund is terminal — no Rework after). A Complaint never mutates the original line: a ready one stays ready and leaves with its Rework, a `picked_up` one stays terminal. **Complaint rate** (Complaints ÷ OrderService lines where `complaint_id IS NULL`, ~1–2%) is a tracked metric on its own Complaints menu/page (the Reports rate panel is a deferred follow-up). The goodwill voucher is **out of band and untracked in v1** — staff issue it manually (no customer-scoped Campaign, and the Complaint no longer records a `voucher_promised` flag). The ~2-day "complain by" window is verbal, **not** system-enforced. See [ADR-0013](docs/adr/0013-complaint-and-rework-line.md) (incl. the 2026-06-30 amendment that removed the status pipeline, and the 2026-09-24 amendment for complaints at the counter).
_Avoid_: ticket, case, dispute.

**Rework line**:
The re-clean of a complained Item, modelled as a **new free `OrderService` line on the same Order** (`price = 0`, `is_priority = true`), carrying a nullable `complaint_id` FK — a non-null `complaint_id` **is** the marker that a line is a Rework. Not a new Order and not a reopening of the original line: it keeps its status, a *new* line is added. A ₀ line adds nothing to pay and needs no new pickup code — the Rework is collected against the **original Order's** code (same Order, same receipt); on an Order still unpaid at the counter, payment still precedes pickup and the Rework does not block it (its 0 cannot be corrected, and it never fills a buy-N-get-M-free slot). While a Rework runs, its Item is not collectable, so a pair turned down at the counter leaves only with the re-clean — one pickup flips both lines to `picked_up`. Each Rework line is logged when it is put on the rack (a `queued` status-log row with no from-status, the acting user), which is what dates a second or third round. A Complaint runs one Rework round at a time: a new round is refused while an earlier one is still in the workshop; a round already `ready_for_pickup` waits on the shelf and leaves with the next in one pickup. Cancelling or refunding an original line that never left the shop (no pickup event) also cancels its Rework lines still in the workshop, in the same transaction (reason `other`, note "Original line cancelled" / "Original line refunded") — even on a paid Order, since the ₀ line moves no money; a Rework already `ready_for_pickup` stays and leaves with the pair. Refunding an original the customer already took home leaves its Rework running: the pair is back in the shop, and that round is how it goes home again. Flows through every existing seam unchanged (queue, pickup-code validation), with one sharpening of the [ADR-0012](docs/adr/0012-photo-precedes-processing.md) photo gate: a Rework counts only Item photos taken **after this round went on the rack** (its opening status-log row; older Reworks without one fall back to the Complaint) — photos from the first visit, from the counter before the pair went home, or from an earlier round are not evidence of the condition it came back in ([ADR-0019](docs/adr/0019-photos-belong-to-the-item.md)). The Order legitimately leaves `completed` and returns as the Rework runs and is collected (`completed_at` shifts to the Rework's pickup; revenue books off `paid_at`, unchanged). **Excluded from the Complaint-rate denominator and the refund picker** — a free Rework must not inflate the grievance that created it, nor be refunded. Operator-facing label is "Rework" (button "Start rework", badge "REWORK"). See [ADR-0013](docs/adr/0013-complaint-and-rework-line.md).
...
_Avoid_: **items processed** — the former name, retired when Item became a real concept. An Item receiving three treatments is **three** Services processed and **one** Item, so the old label counted the wrong noun. Also avoid items completed — completion is pickup, a different event. Both forbidden nouns are gone from the code (2026-09-12): the daily KPI ships `services_processed` and the worker-productivity panel ships `services_processed` / `services_per_hour`, and one query answers both off first arrival at `quality_check`.
...
**Collected** (reports metric):
The cash the counter took on Orders paid inside the stretch, before any refund comes off. It sits between Gross sales and Revenue: Campaign and Voucher discounts are already absorbed (they never reach the amount paid), refunds are not yet. Payment mix, top customers and campaign effectiveness can report **only** this number — all three sum `orders.paid_amount`, and a refund carries neither a payment method nor a Campaign, so there is nothing there to subtract it from. Label those panels Collected.
_Avoid_: using it where Revenue is meant — a figure labelled Revenue has had refunds taken off, and this one has not.
...
**Drop-off photo**:
One per Order, captured on the store iPad at intake. **Proof of the handover, not of the goods**: it records who dropped what off across the counter; the condition of each object is the Item photo's job (decided 2026-09-05). **Required at intake for every Order** — the POS blocks checkout until one is attached, **product-only Orders included** (product-only is a negligible share, so it is not special-cased). Captured before the Order exists and attached immediately after checkout commits; still replaceable later on the order detail page. Not a hard invariant: the post-checkout attach can fail, leaving the Order without one until someone retries — surfaced as "Missing" on the detail page, which is the designed recovery path. See [ADR-0014](docs/adr/0014-dropoff-photo-required-best-effort.md).

**Item photo**:
N per Item, no cap. **The before-service record**: taken at drop-off, mainly by the cashier, as proof of the object's condition before the shop touches it; a worker adds one only on finding a condition worth recording. Shared by every treatment on the Item — one Item receiving three Services has one set of photos, not three. Distinct from the Drop-off photo, which proves the handover rather than the object. Optional per-photo note. Upload is **not** status-gated. **At least one is required to start work**: no OrderService on the Item can transition `queued → processing` while the Item has zero non-deleted photos; a Rework line counts only photos taken after this round went on the rack. Photos **after** treatment are out of scope — the shop shoots those on separate devices for marketing, and this system does not document them. See [ADR-0012](docs/adr/0012-photo-precedes-processing.md) and [ADR-0019](docs/adr/0019-photos-belong-to-the-item.md).
_Avoid_: Service detail photo, service photo (the pre-2026-09-05 name, when photos hung off each treatment), before/after photo (there is no "after" here).
...
- An **Item** owns 0..N **Item photos**; its OrderServices own none. Work on any of its OrderServices starts only once the Item has at least one.
...
- A **Complaint** is opened against one original **OrderService** (FK) and owns **0..N Rework lines** — each Rework a new free OrderService on the *same* Order. Escalation to refund reverses the **original** paid line (`picked_up` or `ready_for_pickup` → `refunded`); a finished ₀ Rework line is left untouched, one still in the workshop is cancelled with it unless the customer had already taken the pair home. See [ADR-0013](docs/adr/0013-complaint-and-rework-line.md).
...
> **Dev:** "Customer wants to collect three of their five Items today, the rest tomorrow. Block them?"
> **Domain expert:** "Allowed, as long as each of the three has every treatment ready. Record an OrderPickupEvent for the three; Order stays open until the rest are collected." (See [ADR-0003](docs/adr/0003-business-rules-locked-2026-04-28.md).)
...
> **Dev:** "The cashier shows the customer the finished pair and they won't take it — the stain is still there. It isn't picked up. Can we open a Complaint?"
> **Domain expert:** "Yes, that's the usual case. Open the Complaint on the ready line and start a Rework. The pair goes back to the workshop, the customer can't collect it until the re-clean is ready, and then it leaves in one pickup with both lines." (See [ADR-0013](docs/adr/0013-complaint-and-rework-line.md), 2026-09-24 amendment.)
...
- **Reports disagree with this glossary on Revenue (closed, 2026-09-12)** — the range panels used to sum pre-discount line values under a Revenue label, so every Campaign and Voucher ever honoured was invisible and refunds were never subtracted. **Resolution:** each panel now reports the basis it is named for. **Revenue**: the daily KPI, the overview trend, the overview's per-store rows, and on the financial panel the revenue figure and the store breakdown. **Gross sales**: the financial gross-sales series and its services/products split, the category treemap and the store × category matrix, and the overview's category and top-service panels. **Collected**: payment mix, top customers, campaign effectiveness. The financial walk-down reads Gross sales − discount = Collected, Collected − refunds = Revenue, Revenue − COGS = Gross profit. Range figures **dropped** when this landed — that is discounts and refunds becoming visible, not a regression. The two query stacks staying separate is a different question, still open in `TODO.md`.
...
- **Photos were per treatment — closed 2026-09-05** — until then a photo belonged to an OrderService, so one object receiving three Services carried three disjoint galleries and cleared three separate start-work gates with what was in practice one cashier shot at drop-off. **Resolution:** photos belong to the **Item**. Rows were re-parented onto the line's Item; older stored files keep their treatment-keyed paths, which is harmless because nothing reads a path's layout. Anything that still says "service photo" or reads photos off a treatment row is wrong. See [ADR-0019](docs/adr/0019-photos-belong-to-the-item.md).
