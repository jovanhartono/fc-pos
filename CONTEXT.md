...
**Voucher code**:
One bearer, single-use string belonging to a Voucher. Globally unique across all Vouchers and deliberately **non-guessable** (a code is money — anyone holding it can redeem it). Minted when the Voucher is created; claimed atomically **when its Order is marked paid** so it can never be spent twice. An Order cancelled before payment never claimed it (discounts resolve at payment — [ADR-0018](docs/adr/0018-price-is-known-or-blank-discounts-at-payment.md)), and a paid refund does not free it. See [ADR-0015](docs/adr/0015-campaign-usage-limit-and-vouchers.md).
_Avoid_: Coupon code, redemption key.

**Usage limit**:
An optional global cap on how many times a **listed** Campaign may be redeemed across all Orders and Stores (e.g. "the first 100 Orders"). Enforced atomically at **payment** — concurrent payments racing the last slot can never both win (the same guarantee Product stock has). A Voucher does not use this field; its cap is its code count. An Order that never pays never consumes a slot ([ADR-0018](docs/adr/0018-price-is-known-or-blank-discounts-at-payment.md)); a paid redemption is not returned by a refund. See [ADR-0015](docs/adr/0015-campaign-usage-limit-and-vouchers.md).
...
_Avoid_: **items processed** — the former name, retired when Item became a real concept. An Item receiving three treatments is **three** Services processed and **one** Item, so the old label counted the wrong noun. Also avoid items completed — completion is pickup, a different event.
...
- An **Order** records every **Campaign** it redeemed; a Voucher redemption additionally names the specific **Voucher code** consumed. Redemption happens at **payment** ([ADR-0018](docs/adr/0018-price-is-known-or-blank-discounts-at-payment.md)) — an unpaid Order carries discount 0 and claims nothing, so an Order cancelled before payment never held a slot or a code, and a paid redemption is never released by a refund.
...
- **Item does not exist in the schema yet (open, 2026-08-08)** — the **Item** entry above records the agreed domain model, not the shipped one. Today `orders_services` itself carries `brand`, `color`, `model`, `size` and a per-line `item_code` (`{order code}-S001`, `-S002`, …), so one pair receiving three Services is three rows, three tag codes, and three copies of the same attributes, with nothing grouping them. Two consequences to be aware of until it lands: **`item_code` identifies a treatment line, not a physical object** — never read it as the thing on the shelf; and nothing prevents recording pickup on one OrderService while a sibling on the same pair is still `processing`. **Resolution:** the glossary is right, the schema is behind — see [ADR-0017](docs/adr/0017-item-groups-order-services.md) for the agreed shape and the migration's cost.
