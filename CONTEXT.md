...
- A **Complaint** is opened against one original **OrderService** (FK) and owns **0..N Rework lines** — each Rework a new free OrderService on the *same* Order. Escalation to refund reverses the **original** paid line (`picked_up` or `ready_for_pickup` → `refunded`); a finished ₀ Rework line is left untouched, one still in the workshop is cancelled with it. See [ADR-0013](docs/adr/0013-complaint-and-rework-line.md).
