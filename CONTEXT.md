...
A User whose job is **collecting** dropped-off items from the customer at intake and **delivering** finished items back after pickup. Logs in solely to open a Shift (attendance); does **not** operate the POS, the queue, or money. Which Orders a Courier collected reads off `intake_channel = 'courier'`, not off the FK alone (see **Intake channel**). Deliberately excluded from the worker-productivity report — that report is an allowlist on `role = worker`, so a Courier never appears. Excluding Couriers from that report is the reason the role exists. See [ADR-0010](docs/adr/0010-courier-role-login-only-excluded-by-allowlist.md).
...
**Intake channel** (`orders.intake_channel`):
How the Items reached the Store: `walk_in` (carried over the counter), `courier` (a Courier fetched them), or `shipped` (a shipping company delivered a parcel). One Order arrived one way, and the column cannot express a fourth. This replaced the older rule that a non-null `collected_by` was itself the delivery marker — that reading broke the moment shipping became a third way in. See [ADR-0020](docs/adr/0020-intake-channel-and-origin-postal-code.md).
_Avoid_: delivery type, order source, `is_delivery`.

**Shipped-in Order**:
An Order whose Items reached the Store by shipping company (JNE, J&T, SiCepat) rather than over the counter or via a Courier — `intake_channel = 'shipped'`. The Customer is usually outside the Store's city, though distance is not the fact being recorded. Sending the finished Items back is **out of band**: no return fee, airway bill, or customer-absent pickup is modelled in v1, and staff handle the return by hand ([ADR-0020](docs/adr/0020-intake-channel-and-origin-postal-code.md)).
_Avoid_: out-of-town order (a Depok customer 20km away may ship one), mail-in order, remote order, paket, kirim.

**Origin postal code** (`orders.origin_postal_code`):
Where the Items came from — the customer's 5-digit kode pos, recorded on Courier and Shipped-in Orders so the shop can see which places generate demand it does not already serve at a counter. A walk-in never carries one. **Optional on both**: blank means the shop does not know, which is a truer answer than a code someone guessed to clear a required field. The kota/kabupaten and provinsi are **derived** by resolving the code through `postal_codes` — reference data keyed by the code, deliberately carrying no `is_active` (the Activation rules govern catalog rows someone drafts and puts on sale; a kode pos is neither). See [ADR-0020](docs/adr/0020-intake-channel-and-origin-postal-code.md).
_Avoid_: origin city (the city is derived, never stored on the Order), address, kode pos as a bare term in schemas and types.
...
**Drop-off photo**:
One per Order, captured on the store iPad at intake. **Proof of the handover, not of the goods**: it records who dropped what off across the counter; the condition of each object is the Item photo's job (decided 2026-09-05). **Required at intake for every Order** — the POS blocks checkout until one is attached, **product-only Orders included** (product-only is a negligible share, so it is not special-cased). Captured before the Order exists and attached immediately after checkout commits; still replaceable later on the order detail page. Not a hard invariant: the post-checkout attach can fail, leaving the Order without one until someone retries — surfaced as "Missing" on the detail page, which is the designed recovery path. (Was "Non-blocking" pre-2026-06-15.) For a **Shipped-in Order** there is no counter to shoot across: the photo is the sealed parcel with its shipping label, taken before opening — better evidence than the contents, since it shows what arrived, from whom, and by which company, and it is the same label the Origin postal code is read off ([ADR-0020](docs/adr/0020-intake-channel-and-origin-postal-code.md)). See [ADR-0014](docs/adr/0014-dropoff-photo-required-best-effort.md).
...
- An **Order** records how its Items arrived in `orders.intake_channel` — `walk_in`, `courier`, or `shipped`. A `courier` Order additionally names the **Courier** who collected them (`orders.collected_by`, FK → a `role = courier` User); a DB CHECK keeps the two in lockstep, so neither can be set without the other. Records **intake** only, never the return leg. See [ADR-0010](docs/adr/0010-courier-role-login-only-excluded-by-allowlist.md) and its 2026-09-10 amendment, and [ADR-0020](docs/adr/0020-intake-channel-and-origin-postal-code.md).
- A **Courier** or **Shipped-in Order** may record an **Origin postal code** (`orders.origin_postal_code`, nullable FK → `postal_codes`); a `walk_in` Order must not. Optional on both — blank is an honest "we don't know". Set at create and correctable on the order detail by anyone who may take an Order — admin, cashier and worker — the same posture `collected_by` has. See [ADR-0020](docs/adr/0020-intake-channel-and-origin-postal-code.md).
...
- **`collected_by` as the arrival marker — closed 2026-09-10** — until shipping became a third way in, a non-null `orders.collected_by` *was* how the system said an Order was not a walk-in, and [ADR-0010](docs/adr/0010-courier-role-login-only-excluded-by-allowlist.md) explicitly rejected a boolean beside it. That was right for two channels and impossible for three. **Resolution:** `orders.intake_channel` carries how the Items arrived; `collected_by` now only says *who* fetched them. A DB CHECK keeps the two agreeing, so existing code reading the FK as "delivered" still returns the right answer — but new code reads the channel, and nothing may treat a null `collected_by` as proof of a walk-in. See [ADR-0020](docs/adr/0020-intake-channel-and-origin-postal-code.md).
