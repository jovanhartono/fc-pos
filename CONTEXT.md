...
**Estimate**:
A price entered at intake that the shop may still revise. Distinct from the catalog question of whether a Service *has* a list price: Repair has none, yet a cashier confident in the number may still enter it as **firm** and take the money at drop-off. Firm is a commitment — if inspection disagrees, the shop absorbs it. An Estimate is not: it stays provisional until confirmed after inspection, and **an Order carrying any unconfirmed Estimate cannot be marked paid**. Payment is binary ([ADR-0001](docs/adr/0001-payment-is-binary.md)), so there is no way to collect the already-known part of such an Order early — it waits. The intake number is kept beside the final one, which is what makes "how far off are our estimates, and whose" a measurable fact rather than an opinion.
Confirming an Estimate is open to **any staff**, and logged. It is deliberately *not* behind the admin money gate: money still moves only through the POS, so the exposure is favouritism rather than shrinkage, and gating something that happens on every Repair would jam the counter. The oversight is the variance report the two stored numbers make possible — estimate versus final, by user — which catches a pattern where a per-order approval would only add friction. See [ADR-0018](docs/adr/0018-estimate-blocks-payment.md).
_Avoid_: quote (fine when talking to a Customer; in the model the term is Estimate), provisional price, TBD price.
...
> **Dev:** "Repair on a bag — the cashier says he's certain it's 300k. Does the Order still have to go out unpaid?"
> **Domain expert:** "No. Certain means firm — take the money at drop-off, it behaves like any other Order. But it's our number now: if the teardown says 500k, we eat it. Not certain? Enter it as an Estimate and the Order stays unpaid until we've looked."
