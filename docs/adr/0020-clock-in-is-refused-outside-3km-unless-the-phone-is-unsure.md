...
- **At 3 km the rings overlap, so a worker can open a Shift against the wrong branch.** The two closest production branches are 2.48 km apart, which the old 1 km ring kept separate. Someone standing at one counter is inside both rings and the gate cannot tell them apart; only the branch they picked on screen is checked. Attendance filed against the wrong branch is now a mistake the gate does not catch.
