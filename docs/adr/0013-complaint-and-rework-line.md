...
- **Refund stays the terminal rung.** A ready line on a paid Order can be refunded (`ready_for_pickup → refunded` was already legal); on an unpaid one the counter cancels it instead ([ADR-0008](0008-cancel-is-unpaid-per-line-refund-twin.md)). After either, no further Rework is offered.
