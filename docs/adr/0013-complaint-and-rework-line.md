...
- **One Rework round at a time.** A new round is refused while an earlier round of the same Complaint is still live (anything short of `picked_up`, `cancelled` or `refunded`, `ready_for_pickup` included), checked under the original line's lock; the Complaint page hides "Start rework" meanwhile. Two live rounds would put the same pair on the rack twice.
