...
**Decision: a Complaint may be opened on a finished line — `ready_for_pickup` or `picked_up`.** Adding a Rework (with the Complaint, or later from the Complaint page) is allowed on the same two statuses. A `ready_for_pickup` line qualifies only once its whole Item is ready — every live treatment on it `ready_for_pickup` — since the customer at the counter is shown the pair, not one treatment; a `picked_up` line always qualifies. Everything else still refuses: unfinished work (`queued`, `processing`, `quality_check`, `qc_reject`), a Rework line, a second Complaint on the same line, and a `refunded` or `cancelled` line.
...
- **Payment still precedes pickup** ([ADR-0009](0009-payment-precedes-pickup.md)). The Order may still be unpaid at the counter. The ₀ Rework line adds nothing to the total and carries a price, so it never blocks the payment. Because prices are still open to correction on an unpaid Order ([ADR-0018](0018-price-is-known-or-blank-discounts-when-priced.md)), the price desk now refuses a Rework line outright — its 0 is fixed. Nor does it count toward a buy-N-get-M-free promo settled at payment: it is not a pair the customer bought, and counting it would hand the free slot to the ₀ line.
- **Refund stays the terminal rung.** A ready line on a paid Order can be refunded (`ready_for_pickup → refunded` was already legal); on an unpaid one the counter cancels it instead ([ADR-0008](0008-cancel-is-unpaid-per-line-refund-twin.md)). After either, no further Rework is offered, and a cancelled original gives the Complaint a derived *cancelled* outcome, so it does not read *pending* forever.
- **A Rework does not outlive an original that never left the shop.** Cancelling or refunding an original line with no pickup event — the counter case — cancels its Rework lines still in the workshop (`queued`, `processing`, `quality_check`, `qc_reject`) in the same transaction, each logged by the acting user with reason `other` and the note "Original line cancelled" or "Original line refunded". They are ₀, so no money moves; on a paid Order this is the one cancel the refund-only rule allows, since nothing else could ever take that line off the rack. Without it the customer gets a free re-clean of work already called off. A Rework already ready is left alone: the re-clean is done and the pair goes home with it. A pair the customer took home and brought back is the other way round: refunding its original leaves the Rework running, because that round is how the pair goes home again — cancelling it would leave the Item reading `picked_up` while the pair sits in the shop, and the pickup desk could never hand it back. A cancelled Rework does not count toward the *reworked* outcome.
- **One Rework round at a time.** A new round is refused while an earlier round of the same Complaint is still in the workshop (`queued`, `processing`, `quality_check`, `qc_reject`), checked under the original line's lock; the Complaint page hides "Start rework" meanwhile. Two rounds in the workshop would put the same pair on the rack twice. A round already `ready_for_pickup` does not block: on a paid Order it can be neither cancelled nor refunded, so a customer who turns the re-clean down again at the counter gets another round, and the ready one waits on the shelf and leaves with it in one pickup, as the original line does.
- **Every Rework round is logged when it is put on the rack.** A line has no `created_at`, so each new Rework line gets one `order_service_status_logs` row (`from_status` NULL → `queued`, the acting user, "Rework for complaint #N"). That row is what dates a second or third round on the timelines, and what the photo gate measures from: a round counts only Item photos taken after this round went on the rack, so a counter photo from before the pair went home, or round 1's photos, cannot start a later round ([ADR-0019](0019-photos-belong-to-the-item.md), 2026-09-25 amendment). No report reads it: every reader of that table filters on `to_status` `processing`, `quality_check` or `qc_reject`. Reworks created before this amendment have no such row; screens date the first round by the Complaint and leave later rounds undated.
