...
const app = new Hono()
  .get('/', async (c) => {
    const users = await db.query.servicesTable.findMany({
  ...
  .get('/:id', idParamSchema, async (c) => {
    ...
    const user = await db.query.servicesTable.findFirst({
    ...
    return c.json(success(user, 'Service retrieved successfully'));
  })
