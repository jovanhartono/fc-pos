...
type ServiceForm = z.input<typeof POSTPaymentMethodSchema>
type ServiceFormProps = FormOptions<ServiceForm>
const props = defineProps<ServiceFormProps>()
const emit = defineEmits<{
  submit: [data: z.infer<typeof POSTPaymentMethodSchema>]
}>()

const { handleSubmit, isSubmitting } = useForm({
  ...props,
  validationSchema: POSTPaymentMethodSchema,
})
  ...
  <form @submit="onSubmit" class="flex flex-col gap-y-3">
    <FormField v-slot="{ componentField }" name="code">
      <FormItem>
        ...
        <FormControl>
          <Input placeholder="Code" v-bind="componentField" />
        </FormControl>
        <FormMessage />
      </FormItem>
    </FormField>
    ...
    <FormField v-slot="{ componentField }" name="name">
      <FormItem>
        <FormLabel asterisk> Payment Method </FormLabel>
        <FormControl>
          <Input placeholder="Payment method" v-bind="componentField" />
        </FormControl>
        <FormMessage />
      </FormItem>
    </FormField>
